...
void AppTemp_GetData(SystemData* sys)
{
	...
	float temp = sys->meteo->temp / 100.0f;
	float humid = sys->meteo->humid / 100.0f;
	float wind_speed = sys->meteo->wind_speed / 100.0f;
	...
	if (apptemp >= 0.0f)
		sys->meteo->apptemp = (int16_t)(apptemp * 10.0f + 0.5f);
	else
		sys->meteo->apptemp = (int16_t)(apptemp * 10.0f - 0.5f);
}